...
> 텍스트를 3글자씩 겹쳐 자른 트라이그램으로 `LIKE`·정규식·유사도 검색을 GIN/GiST 인덱스로 가속하는 **PostgreSQL 공식 contrib 모듈**. `pg_bigm`(2-gram)보다 지원하는 기능이 훨씬 많지만, 2글자 검색어와 한글 정규식에서는 인덱스가 오히려 독이 된다.
