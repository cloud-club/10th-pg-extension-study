...
up() {
  ...
  wait_healthy
  ctr psql -X -U "$DBUSER" -d "$DB" -c "SELECT version();"
}
...
ensure_up() {
  if [ -n "$(cid)" ]; then wait_healthy; return 0; fi
...
run_file() {  # $1 = 호스트 경로 (예: sql/01-x.sql), 컨테이너에는 /lab/<dir>/ 로 마운트됨
  ...
  ctr psql -X -e -v ON_ERROR_STOP=1 -U "$DBUSER" -d "$DB" -f "/lab/$dir/$base" 2>&1 | prettify
}
...
case "${1:-all}" in
  ...
  all)     cat LESSON.md; up; run_dir migrations; run_dir sql
           echo; cyan "-- 끝 --"
  ...
  sql)     cat LESSON.md; ensure_up; run_dir sql ;;
  psql)    ensure_up
           printf '\033[2m  [호스트]   $\033[0m \033[36m%s\033[0m\n' \
  ...
  *) echo "usage: ./run.sh [all|up|explain|migrate|sql|psql|shell|logs|down|<번호>]" >&2; exit 1 ;;
esac
